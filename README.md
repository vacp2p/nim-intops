...
2. Support 64- and 32-bit CPUs.
3. Support Windows, Linux, and macOS.
4. Support gcc and clang.
5. Support runtime and compile time usage.
6. Offer the best implementaion for each combination of CPU, OS, C compiler, and usage time.
7. Allow the user to pick the implementation manually.
8. Future addition of new operations or implementations must not require library reorganization.
...
### Improving an optimal implementation picking algorithm

When you invoke a primitive, it decides which of its implementation to call with the given environment. This logic is described in templates at `ops/{op}.nim`. So, to improve this logic, locate the operation and the template you want to modify and make your edits.

To define logic branches, use the global constants defined in `consts.nim`. If necessary, define new constants.
...
1. define the pure Nim implementation of this flavor in `impl/pure.nim`
1. define a template for the operation in `ops/{op}.nim`; the template should just invoke the pure implementation.

For example, let's say we want to add a new kind of addition called magic addition.

In `impl/pure.nim` we add:
...
1. define a new function in `impl/{impl}.nim`
2. update the logic that picks the best implementation in `ops/{op}.nim`

For example, let's implement magic addition in C.

In `impl/inlinec.nim` we add:
